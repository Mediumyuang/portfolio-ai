...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    // Lower pixel ratio for better performance (max 1.5 instead of 2)
    ...
    const pointerThrottle = 16; // ~60fps updates
    
    const handlePointerMove = (event: PointerEvent) => {
      const now = performance.now();
      if (now - lastPointerUpdate < pointerThrottle) return;
